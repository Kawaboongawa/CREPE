...
__global__ void complementary_kernel(PtrStepSzb src, PtrStepb dst)
{
	uint index = blockIdx.x * blockDim.x + threadIdx.x;
	int x = threadIdx.x + blockIdx.x * blockDim.x;
	int y = threadIdx.y + blockIdx.y * blockDim.y;
...
void complementary_caller(PtrStepSzb src, PtrStepb dst)
{
	dim3 threads(16, 16);
	dim3 blocks((src.cols + threads.x - 1) / threads.x, (src.rows + threads.y - 1) / threads.y);
	complementary_kernel << <blocks, threads, 0, 0 >> > (src, dst);
}

__global__ void get_AND_matrix_kernel(PtrStepSzb src1, PtrStepb src2, PtrStepb dst)
{
	uint index = blockIdx.x * blockDim.x + threadIdx.x;
	int x = threadIdx.x + blockIdx.x * blockDim.x;
	int y = threadIdx.y + blockIdx.y * blockDim.y;
...
{
	dim3 threads(16, 16);
	dim3 blocks((src1.cols + threads.x - 1) / threads.x, (src1.rows + threads.y - 1) / threads.y);
	get_AND_matrix_kernel << <blocks, threads, 0, 0 >> > (src1, src2, dst);
}